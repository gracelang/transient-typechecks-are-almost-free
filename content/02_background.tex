...
\SOMns provided a good foundation for a new Grace implementation.
And allowed us to gain performance on the level of V8,
Google's JavaScript implementation
(cf. \cref{sec:baseline-perf} and \citet{Marr2016}).
\SOMns was changed to parse Grace code
and use the slightly adapted Truffle tree nodes to realize the Grace semantics.
Moth is mostly compliant to the specification.
One exception is that it still supports the \texttt{nil} value of Newspeak.
\sm{add code macro}
