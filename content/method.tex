...
For AST interpreter,
we can implement this approach by attaching the corresponding checks to the relevant AST nodes:
...
The check node uses the type (cf. \cref{ex:type} \cref{ex:type:check}) to test whether an observed
object conforms to it.
An object satisfies the type, if all members required by the type are provided
