...
--- i.e.\ i.e.\ Grace's static type checking is optional, and so an
...
students shouldn't be encouraged to improve performance by removing
...
While Grace's static type system supports full static type checking
...
\code{Department}) even the the body of the \code{registerTo} method
...
\kjx{why both reading and writing?  answer - doesn't matter?}
...
object conforms to it.   These \code{Type} objects are created by
Grace \code{interface} expressions, and were already present in Moth
...
Furthermore, the \code{static\_guard} code is only executed when selecting
