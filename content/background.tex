...
dynamic type checking\mwh{I'm not convinced this is what Unknown does.}\sm{perhaps better: the type system
includes a distinguished ``Unknown'' type, which matches any other type and this is the implicit type for untyped program parts.}).
...
Grace, \ins{the existing implementations that use type information}
\del{all extant implementations} have the unfortunate property that
adding type declarations to a program makes that program run slower\mwh{This is simply not true. Most extant implementations have no performance impact whatsoever from adding type declarations, because they disregard them entirely.}\sm{better after applying the proposed edit?}---teaching students that \emph{removing} type declarations is an
effective optimization technique.
Furthermore, this property is shared
by other optionally typed languages including \ugh{Groovy\citep{Muehlboeck2017}}\sm{This is still not true. Muehlboeck2017 doesn't say anything about groovy. At least I can't find the word Groovy in the text with my pdf reader.
