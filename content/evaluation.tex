...
The goal was to complement our benchmarks with additional that are
...
Overall, we see a peak-performance overhead of only 
...
in a way that introduces checks that do not coincide with any operations
on the relevant objects that have been performed before.
We consider this benchmark a pathological case and discuss this example
in detail in \cref{sec:disc-pathological-case}.


Indication that results of \citet{Richards2017}
are also applicable to highly optimizing language implementations

% explain minima maxima.

% reference optimization techniques?
% reference spec
