...
we compare Moth with all checks disabled, \ie, the baseline version from 
\cref{sec:baseline-perf}, with a Moth that has all checks enabled.
...
Richards has as one major component also a linked list traversal,
similar to List.
...
Some benchmarks show however performance improvements,
so that the run time decreased.
Notable are here Permute (\OverheadPermuteP),
GraphSearch (\OverheadGraphSearchP), and Storage (\OverheadStorageP).
Permute simply creates the permutations over an array.
