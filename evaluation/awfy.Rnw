...
data_4 <- select(load_data_file("010-f8d6ca79-moth.data.bz2"),
                 c("Value", "Unit", "Benchmark", "VM", "Iteration"))
data_4 <- filter(data_4, VM != "MothTyped")
...
data_2 <- filter(data_2, Iteration >= warmup_low & Iteration < warmup_high)
data_4 <- filter(data_4, Iteration >= warmup_low & Iteration < warmup_high)

data <- rbind(data_2, data_4, data_5)
...
\newcommand{\WarmupCutOff}{\Sexpr{warmup_low}\xspace}
