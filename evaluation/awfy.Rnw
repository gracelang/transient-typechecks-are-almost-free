...
data <- select(load_data_file("023-a3e099e1-full-run.data.bz2"),
...
\def\AwfyBaseline{%
<<awfy-baseline, fig.height=2.5, fig.width=4.5>>=
...
p <- p + ylab("Run-time factor, normalized to Java (untyped)\n(lower is better)") # +  xlab("Cores")
p
@
}%
