...
Furthermore, we present the results for the first iteration as well as iteration 100.
The first iteration is executed at least partially in the interpreter,
while iteration 100 executes compiled.
...
However, for iteration 100, the confidence intervals overlap
for the optimized Moth as well as the one that does not perform typechecks.
This means, the that Moth does not suffer from a general linear overhead for adding
type checks.
...
\begin{figure}
  ...
  \caption{Transient Typechecks are (Almost) Free.
    ...
    For iteration 100 however, the compiler eliminated the overhead of type checks
    and both Moth variants have essentially the same performance
