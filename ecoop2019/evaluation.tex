...
\smtodo{revisit this sentence once done:}
...
% Graal 0.33 Feb. 2018
\smtodo{update these details when we have time... not a rejection criterion,
...
The overall results comparing Moth with all optimizations
with the version that does not do any type checks
are depicted in \cref{fig:typing-overhead}.
...
\begin{figure}[htb]
  ...
  The performance of Moth with both optimization and Moth
...
\Cref{fig:type-cost-micro} shows such a common scenario of methods being 
gradually annotated with types
does not incur an run-time overhead in our system.
The plot shows the median of the run time for each benchmark configuration.
Furthermore, it indicates a band with the median absolute deviation to
see the range of which the measurements vary.
...
which we chose to ensure that some details remain legible for the faster
variants.

For Moth (both) and Moth (untyped), the bands of median absolute deviation overlap
for the optimized Moth as well as the one that does not perform typechecks.
This means, the that Moth does not suffer from a linear overhead for all of its
type checks.
However, as previously argued for the List benchmark,
this is only the case for checks that can be subsumed by shape checks that
were performed already before adding type checks.

\begin{figure}
	\TypeCost{}
  \caption{Transient Typechecks are (Almost) Free.
    Two microbenchmarks demonstrating the common scenario that the addition
    ...
    Note that the y axis has a logarithmic scale to preserve details for the optimal case.
    Moth (both) and Moth (untyped) have on both benchmarks essentially the same performance
