...
\smtodo{revise in the context of the previous sections. perhaps reduce redundancy/verbosity}
...
we show the corresponding impact on performance
and how the optimizations enable a just-in-time compiler
...
\Cref{fig:perf-impact-optimization} shows how Moth performance on average over
...
performance of unit tests, which has an important impact on developer productivity.
...
\Cref{fig:type-cost-micro} shows the common scenario that additional type annotations
in our system do not incur an run-time overhead.
The used benchmarks Check and Nest have at their core methods with 5 parameters.
