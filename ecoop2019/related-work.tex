...
\mwh{I've checked with both the papers and the implementation. It
  ...
  1) The transient semantics does check field values (but there are
  no fields in Grace)
  2) They check the presence of methods by name, but not their
  signatures, like in Moth
  3) They also check return values against the type expected in the
  caller
  4) They do not check the values inside lists (etc) eagerly, but
  do check when they are retrieved
  5) They do a lot of dataflow analysis to remove unnecessary
  checks statically}
%
