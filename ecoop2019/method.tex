...
\mwh{This reference doesn't discuss
typing at all; was it meant to be the brands paper? Or perhaps one
of Tim's Tinygrace papers?}\kjx{Tim's thesis}
~here we explain how
...
and then annotates the \code{printRegistration} method's
...
cannot respond to the ``registration'' message.
...
\code{Vehicle} because that check considers only that the
vehicle has a \code{registerTo} method,
but not the required argument type of that method.
At \cref{ex:invoke-register-to} the developers 
...
(the object that is passed in is not a \code{Department})
even though the body of the \code{registerTo} method
...
We have our implemented gradual type checks 
by extending Moth, 
