...
elsewhere\citep{TimJonesThesis}\mwh{This reference doesn't discuss
typing at all; was it meant to be the brands paper? Or perhaps one
of Tim's Tinygrace papers?}\kjx{Tim's thesis}; here we explain how
...
writes of variables
(see \$\ref{reticRW}).
\kjx{can Michael check this?}\mwh{Return values are checked on both
sides. Local variables can be assigned any value, but are otherwise
checked against their inferred types. Grace does not have fields.
}
